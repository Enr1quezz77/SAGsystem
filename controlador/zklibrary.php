    ...
    public function getAttendance()
    {
        ...
        try {
            ...
            if (count($this->attendance_data) > 0) {
                ...
                $attendance_data = substr($attendance_data, 10); // Restaurar a 10 para firmware B&W nativo
                
                // Formato de 16 bytes (Dispositivos tipo Black & White y Serie A)
                if (strlen($attendance_data) % 16 == 0 || (strlen($attendance_data) > 16 && strlen($attendance_data) % 40 != 0)) {
                    while (strlen($attendance_data) >= 16) {
                        $u = unpack('vpad1/vuid/vpad2/Vtime/Cstate/Cverif/Vpad3', substr($attendance_data, 0, 16));
                        if ($u !== false && $u['time'] > 0) {
                            // PHP 8 safe time decoding using existing bitwise logic without E_DEPRECATED notices
                            $timestamp = @$this->decodeTime($u['time']);
                            // Para compatibilidad retornamos el array estándar
                            array_push($attendance, array((int)$u['uid'], (string)$u['uid'], (int)$u['state'], date('Y-m-d H:i:s', strtotime($timestamp))));
                        }
                        $attendance_data = substr($attendance_data, 16);
                    }
                } else {
                    // Cargar Formato TFT Estandar 40 bytes
                    $attendance_data = substr($attendance_data, 2); // compensate the original substr 10
